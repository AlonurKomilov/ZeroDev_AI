...
class UpgradeRequest(BaseModel):
    ...
    prompt: str = Field(
        ..., example="Refactor the database layer to use the Repository pattern."
    )
...
@router.post(
    "/upgrade/start", response_model=JobResponse, status_code=202, tags=["Upgrade"]
)
async def start_upgrade(
    req: UpgradeRequest,
    orchestrator: TransformationOrchestrator = Depends(
        lambda: transformation_orchestrator
    ),
) -> JobResponse:
    """
    ...
    job_id = orchestrator.start_transformation_workflow(
        repo_url=req.repo_url, prompt=req.prompt
    )
