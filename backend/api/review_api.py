...
from backend.core.orchestration_service import orchestration_service, ModificationState
...
class ApplyResponse(BaseModel):
    job_id: str = Field(..., description="The ID of the modification job.")
    status: str = Field(..., description="The final status after attempting to apply the patch.")
...
def get_diff_for_review(
    *,
    job_id: str = FastApiPath(..., description="The ID of the modification job to review."),
    ...
    if job["state"] != ModificationState.AWAITING_APPROVAL:
        raise HTTPException(status_code=400, detail=f"Job is in state '{job['state']}', not awaiting approval. Cannot fetch diff.")
...
def apply_reviewed_patch(
    *,
    job_id: str = FastApiPath(..., description="The ID of the modification job to apply."),
    current_user: User = Depends(current_active_user),
):
    """
    ...
    if job.get("user_id") != str(current_user.id):
        raise HTTPException(status_code=403, detail="You are not authorized to apply this patch.")

    if job["state"] != ModificationState.AWAITING_APPROVAL:
        raise HTTPException(status_code=400, detail=f"Patch cannot be applied. Job is in state '{job['state']}', not awaiting approval.")
    ...
    if result["success"]:
        ...
        return ApplyResponse(job_id=job_id, status="success", message="Patch applied successfully.")
    else:
