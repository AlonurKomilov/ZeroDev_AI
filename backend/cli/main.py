...
def init(project_name: str):
    ...
    console.print(
        f"Scaffolding new project: [bold magenta]{project_name}[/bold magenta]..."
    )
    scaffold_project(Path.cwd() / project_name)
    console.print(
        f"✅ Project [bold green]{project_name}[/bold green] scaffolded successfully!"
    )
...
async def generate(project_path: str):
    ...
    console.print(
        f"⚙️ Generating code for project in [bold magenta]{project_dir}[/bold magenta]..."
    )

    if not config_path.exists():
        console.print(
            f"❌ [bold red]Error:[/bold red] `config.yaml` not found in {project_dir}"
        )
        return
...
def ci_cd(project_path: str):
    ...
    project_dir = Path(project_path)
    console.print(
        f"🚀 Setting up CI/CD for [bold magenta]{project_dir.name}[/bold magenta]..."
    )
...
def deploy(project_path: str, port: int, push: bool, no_health: bool):
    ...
    project_dir = Path(project_path)
    console.print(
        f"🚢 Deploying project [bold magenta]{project_dir.name}[/bold magenta]..."
    )
    deploy_project(
...
def rollback(file_path: str, version: str):
    ...
    console.print(
        f"⏪ Rolling back [bold magenta]{file_path}[/bold magenta] to version [bold yellow]{version}[/bold yellow]..."
    )
